 ...
 *
 * Le décodeur est conçu pour être :
 *   - extrêmement rapide (ISR en IRAM)
 *   - non bloquant
 *   - robuste face aux timings irréguliers
 *   - compatible avec les spécifications NMRA
 *
...
bool DccDecoder_getEvent(DccEvent &ev)
{
    ...
    if (ok && DEBUG_LEVEL >= DEBUG_VERBOSE)
    {
        if (ev.type == DCC_EVT_BIT)
            LOG_VERBOSE("DCC Event → BIT=%u phase=%u dt=%lu", ev.bit, ev.phase, ev.dt_us);
        else if (ev.type == DCC_EVT_CUTOUT_START)
            LOG_VERBOSE("DCC Event → CUTOUT_START dt=%lu", ev.dt_us);
        else if (ev.type == DCC_EVT_CUTOUT_END)
            LOG_VERBOSE("DCC Event → CUTOUT_END dt=%lu", ev.dt_us);
    }
